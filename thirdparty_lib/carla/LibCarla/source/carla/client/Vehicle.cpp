...
namespace client {
  ...
  void Vehicle::ApplySteeringMode(rpc::VehicleSteeringMode steering_mode) {
    const uint8_t mode = static_cast<uint8_t>(steering_mode);
    GetEpisode().Lock()->ApplySteeringModeToVehicle(*this, mode);
    _control.steering_mode = mode;
  }
