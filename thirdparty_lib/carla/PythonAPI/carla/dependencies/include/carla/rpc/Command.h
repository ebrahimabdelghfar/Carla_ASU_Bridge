...
namespace rpc {
  ...
  public:
    ...
    struct ApplyVehicleSteeringMode : CommandBase<ApplyVehicleSteeringMode> {
      ApplyVehicleSteeringMode() = default;
      ApplyVehicleSteeringMode(ActorId id, uint8_t value)
        : actor(id),
          steering_mode(value) {}
      ActorId actor;
      uint8_t steering_mode = static_cast<uint8_t>(VehicleSteeringMode::FrontAckerman);
      MSGPACK_DEFINE_ARRAY(actor, steering_mode);
    };
