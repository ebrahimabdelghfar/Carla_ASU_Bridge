...
class CarlaOdometry {
 public:
  ...
  // Bind the GPS sensor whose stream feeds position in "gnss" mode. Only used
  // by position_source(); get_state() still takes the pointer as an argument.
  void bind_gps(CarlaGPS* gps) { gps_ = gps; }

  // Identity of the measurement the position currently comes from.
  //
  ...
  //
  // Returns true if the position source is tracked separately (gnss mode),
  // setting `frame`/`sim_time` to that measurement's identity; false if the
  // caller's own world frame is the correct identity.
  bool position_source(uint64_t& frame, double& sim_time) const;

  // Dead-reckon the last state built by get_state() forward by dt seconds.
  //
  ...
  // is drawn — the prediction inherits whatever noise the source state
  // carries, so a noise-enabled config does not random-walk between real
  // samples. Returns nullopt if no state has been produced yet.
